"""
justjoin-scraper — 2captcha Scraper API edition (fourth engine)

Fetches one page through the 2captcha Scraper API, which renders it on
2captcha's infrastructure and returns the result over plain HTTPS, and
feeds it to this project's product_parser.

When this path is the right one
-------------------------------
Rarely on this site, and saying so is the honest sentence.

justjoin.it's offers endpoint answers with JSON that needs no browser at
all, and it answered every request measured on 2026-09-18 — from a bare
netcup datacentre address, to `curl` with no headers, to `wget`, with
sixteen candidate challenge markers counting zero across seven captures.
The Scraper API's own exits are datacentre addresses too, and nothing here
needs an exit this site scores, because nothing measured suggests it scores
exits at all.

So use this client when you want someone else's infrastructure to run the
browser, not because the site demands it. What it genuinely buys here is
...
the rendering buys nothing there.
    ...
    # $JUSTJOIN_CDP_ENDPOINT, so neither needs to be typed — a secret in
...
from product_parser import (BOT_CHALLENGE_MARKERS, MODES, DEFAULT_MODE,
                            ...
                            parse_page, parse_detail_response,
...
def _run_once(args, attempt: int = 1, attempts: int = 1) -> int:
    ...
    state = detect_page_state(html, upstream_status, args.url,
                              args.mode,
                              getattr(args, "route", DEFAULT_ROUTE))
    if state == "blocked":
        dump = f"{args.out}_scraperapi_debug.html"
        with open(dump, "w", encoding="utf-8") as f:
            f.write(html)
        logger.error(
            "justjoin.it did not serve the Scraper API's request (upstream HTTP "
            "%s, %d bytes) — saved to %s. This is NEW: on 2026-09-18 this "
            "site served every route to a bare datacentre address with no "
            "challenge of any kind, so there is no measured remedy to "
            "recommend and the saved HTML is the evidence for what changed. "
            "--cdp-url routes the request through a Scraping Browser session "
            "with a country segment, which is the usual next move. This is "
    ...
    if vendor:
        ...
        logger.error("A challenge page is not a final answer — retry before "
                     "concluding anything (--retries). justjoin.it rendered no "
                     "challenge to any request measured on 2026-09-18, so "
                     "there is no measured remedy here — --cdp-url routes "
                     "through a Scraping Browser session with a country "
    ...
    products = parse_page(html, mode=args.mode,
                          route=getattr(args, "route", DEFAULT_ROUTE),
                          url=args.url, page=1)
    # NO per-row category assignment here, and its removal is a fix rather
    # than an omission. This client inherited `row.category = args.category`
    # from the repo it was copied from, where the row class has no
    # `category` field either — so it set an attribute that `asdict()` never
    # reads and that reached neither the JSON nor the CSV. It looked like it
    # was labelling rows and was doing nothing at all (CLAUDE.md §16: a
    # copied file is the one with no evidence behind it).
    #
    # `--category` labels the RUN, and the sidecar is where a run-level fact
    # belongs; `finish_run` already records it. Checked before removing: two
    # siblings (bbb, montblanc) do carry a real `category` column and their
    # identical line is correct there, so this is not a family-wide fix.
...
def parse_args():
    p = argparse.ArgumentParser(
        description="justjoin.it scraper — 2captcha Scraper API edition (no local "
                    ...
                    "datacenter addresses and justjoin.it refuses them — it does "
                    "not, as measured.")
    ...
    p.add_argument("--url", default=None,
                   help="A justjoin.it URL: a listing page "
                        "(/job-offers/...) or one offer "
                        "(/job-offer/{slug}). "
                        "/jobs, or /jobs/{id}-{slug} with --mode job. Used if "
                        "endpoint answers that for free. Required, unless "
                        "JUSTJOIN_URL is set in the environment or .env.")
    ...
    p.add_argument("--out", default="justjoin_jobs_scraperapi",
    ...
    env_config.apply(args, keys={
        "TWOCAPTCHA_KEY": "key",
        "JUSTJOIN_CDP_ENDPOINT": "cdp_url",
        "JUSTJOIN_URL": "url",
    })
    if not args.url:
        p.error("no --url given, and JUSTJOIN_URL is not set in the environment "
                "or in .env.")
    return args
