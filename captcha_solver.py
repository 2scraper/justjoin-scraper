...
WHAT THIS MODULE IS FOR ON JUSTJOIN.IT
--------------------------------------
**Nothing, as measured — and the honest version of that sentence is
narrow.**

On 2026-09-18, from a netcup datacentre address (AS197540, Nuremberg),
justjoin.it served every route this scraper reads. Sixteen candidate
challenge markers were counted across seven captures — the homepage, two
rendered listings, a filtered listing, an offer page, an endpoint response
and the site's own 404 — and every one was zero. Identical 200s came back
from `curl` with no headers at all, from `python-requests` and from `wget`.
No challenge has ever been rendered to this scraper.

But "no challenge rendered" is NOT "no captcha configured" (CLAUDE.md §18),
and this site has one:

  * **justjoin.it runs Google reCAPTCHA v2, on its APPLICATION FORM.** It
    publishes the key itself, in the Next.js runtime config that every page
    carries:
    ...
    references it lives in the job-application form's i18n namespace
    ("reCAPTCHA verification failed. Please try again."). So the widget
    guards submitting a CV, which this scraper never does.

    That was measured rather than reasoned. In a live headless Chromium,
    seven seconds after load:

        route                            grecaptcha   iframes   sitekeys
        /job-offers/all-locations        undefined    0         0
        /job-offer/{slug}                undefined    0         0
        /api/candidate-api/offers        undefined    0         0

    Not merely un-rendered: `___grecaptcha_cfg` is absent and no captcha
    script is loaded at all on the read path. The key in the config is the
    only trace of it.

    This is the INVERSE of the shape a sibling repo found, and both are
    worth carrying. There, the sitekey appeared in no served HTML and only
    a live browser revealed it. Here it is in every served page and a live
    browser reveals nothing. So §18's "grep a good page for the site's own
    captcha config" works on this site — and a reader who stopped at the
    grep would wrongly conclude a widget is waiting on the listing.
    `product_parser.site_recaptcha_sitekey` reads that key, and this
    module would use it if a challenge ever rendered.

  * **Cloudflare fronts the site and has never challenged it.** The
    response headers name `server: cloudflare` on every route including the
    upstream API host's 503s. No `/cdn-cgi/challenge-platform` reference
    appears on any capture, and `challenges.cloudflare.com` is zero on all
    seven. See `product_parser.BOT_CHALLENGE_MARKERS` for the two candidate
    markers that were counted and thrown out.

So this module is insurance on justjoin.it rather than a route in use. The
...
challenging is met with tools that already work, and every one of its
figures comes from a sibling repo rather than from here.

What is NOT true, and must never be written, is that a captcha here "cannot
be solved". 2captcha solves ordinary reCAPTCHA v2
(`RecaptchaV2TaskProxyless`) — which is exactly what this site
configures — as well as enterprise reCAPTCHA
(`RecaptchaV2EnterpriseTaskProxyless`), v3 and Cloudflare Turnstile
(`TurnstileTaskProxyless`), and this module builds all of them. The only
sentence this repo is entitled to is which task types it implements
(CLAUDE.md §19).

PROVENANCE OF THE FIGURES BELOW
-------------------------------
This file is family core and carries measurements taken on SIBLING sites.
They are evidence about the SOLVER, not about justjoin.it, and are labelled as
